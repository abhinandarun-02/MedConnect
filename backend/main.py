...
@app.route('/predict')
def predict():
    # Get the symptoms from the request parameters
    symptoms = request.args.get('symptoms')

    # Check if symptoms are provided
    if symptoms is None:
        return jsonify({'error': 'Please provide symptoms as a comma-separated list'}), 400

    # Call the predictDisease function to make predictions
